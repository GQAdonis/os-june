...
pub async fn connectors_apply_runtime(
    ...
    restart_live_hermes_modes(&app, &bridge).await
}
...
/// Applies local runtime configuration to every live mode by restarting each
/// process with its current write mode and cwd. This is a no-op when no Hermes
/// runtime is running; the next start renders the fresh config/env/sandbox.
async fn restart_live_hermes_modes(app: &AppHandle, bridge: &HermesBridge) -> Result<(), AppError> {
    ...
    // connector or local capability settings change would silently relocate the
    // agent's files.
    ...
    for (full_mode, cwd) in connections {
        ...
        // runtime, then re-start it so per-spawn config, environment, and
        // sandbox grants are re-rendered.
        stop_hermes_mode(bridge, full_mode)?;
    ...
    // Do not call Hermes' `/api/gateway/restart`: upstream launches a replacement
    // outside June's supervision, which loses June-owned spawn configuration such
    // as the selected Obsidian vault environment variable. The bridge-owned
    // stop/start above is the complete restart operation.
    Ok(())
}
