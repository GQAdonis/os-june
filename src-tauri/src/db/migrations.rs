...
pub async fn run_migrations(_pool: &SqlitePool) -> Result<(), sqlx::error::Error> {
    ...
    for statement in include_str!("../../migrations/014_routine_browser_grants.sql").split(';') {
        let statement = statement.trim();
        if !statement.is_empty() {
            query(statement).execute(_pool).await?;
        }
    }
    for statement in include_str!("../../migrations/015_browser_outcome_ledger.sql").split(';') {
        let statement = statement.trim();
        if !statement.is_empty() {
            query(statement).execute(_pool).await?;
        }
