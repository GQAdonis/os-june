...
- **Local mode (default).** You authorize Google *on your Mac*; the keys stay in your Mac's Keychain; every API call comes from your device. OpenSoftware's servers are not in the data path — we don't hold anything that could read your mail even under compulsion. This covers the large majority of real usage.
