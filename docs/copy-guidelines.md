...
**The path:** app → Scribe API (the backend, running in an Intel TDX confidential VM, the "TEE") → Venice. Everything goes through Venice. The attestation chain (README.md, and the `/verify` page served from inside the TEE) proves the backend runs the exact open source code we publish. It verifies the code in the VM, not what upstream model providers do. (Source: `README.md:19`, `scribe-api/crates/api/src/handlers/verify.rs`.)
...
- **Anonymous (opt-in):** the request is still routed and anonymized by Venice, identity stripped, but the underlying model provider may retain prompts under its own policy.
...
- **"Everything leaving the TEE for model inference goes through Venice. By default it runs on Venice private models: zero data retention, no training."** The canonical routing explanation (long form in `README.md:19`).
