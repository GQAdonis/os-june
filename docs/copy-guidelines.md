...
**The agent's sandbox (the seatbelt):** on macOS, June wraps the agent runtime in a kernel sandbox (Apple's Seatbelt, via `sandbox-exec`) at spawn, and every subprocess the agent starts inherits it. It is a write-jail: all writes are denied, then re-allowed only inside the agent's own workspace, runtime, and temp directories. Reads stay broadly open on purpose (the agent is useful because it can read your files), except a secret-read denylist: SSH keys, cloud credentials (`~/.aws`, `~/.config/gcloud`, `~/.config/gh`), Keychains, `.netrc`, `.git-credentials`, and similar are unreadable even though everything else is. Sessions are **Sandboxed by default**; **Unrestricted** is a per-session opt-in behind a confirmation dialog, and in Unrestricted the jail and the secret denylist are both off. The sandbox and the approval prompts are two separate layers: the jail blocks writes at the kernel level, approvals govern what the agent does with everything else. (Source: `src-tauri/src/hermes_bridge.rs` `build_sandbox_profile` and `sandbox_write_roots`; `src/lib/agent-session-modes.ts`; the mode picker copy in `src/components/agent/AgentWorkspace.tsx`.)
...
| "The sandbox makes the agent safe" | The sandbox is a write-jail, not a safety guarantee: the agent still reads your files, still acts outwardly with your approval, and can still be wrong. It is one layer; approvals are the other | "Sandboxed by default: writes outside the agent's own workspace are blocked at the kernel level. Approvals govern everything else." | `hermes_bridge.rs`, `LearnSteps.tsx` |
| "The agent can't touch your files" | Sandboxed sessions read files by design (that is what makes the agent useful); only writes are jailed, and Unrestricted lifts the jail entirely | "Sandboxed: June can read your files but only change its own workspace. Unrestricted: June can change any file your account can." (the approved mode copy, verbatim) | `AgentWorkspace.tsx` SANDBOX_OPTIONS |
| "Your credentials are always protected from the agent" | The secret-read denylist (SSH keys, cloud credentials, Keychains) only exists while the jail is on; Unrestricted sessions have no denylist | "In sandboxed sessions, credential stores like SSH keys and Keychains are unreadable. Unrestricted turns that protection off, by your choice." | `hermes_bridge.rs` secret-read denylist |
...
- **"June can read your files but only change its own workspace."** (Sandboxed) and **"June can change any file your account can."** (Unrestricted). The approved mode descriptions; quote them exactly.
- **"Blocked at the kernel level, not just by policy."** The sandbox enforcement claim, for sandboxed sessions only.
...
- **Sandboxed / Unrestricted** are the two per-session agent modes (capitalized in UI). "Seatbelt" is the macOS mechanism name: fine in technical docs, not in consumer copy (say "sandbox" or "a kernel-level sandbox"). Never call the sandbox a TEE or vice versa: the sandbox runs on the user's device around the agent; the TEE is our backend's confidential VM.
