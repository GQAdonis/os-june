...
You authorize Google, Notion, or Linear on your Mac. The provider grant is
stored in your Mac's Keychain. Every Gmail, Calendar, Notion, and Linear API
call June makes originates from your device, using that grant, and goes
straight to the provider. June API is not involved in connector calls.

## What OpenSoftware can and cannot see

**Cannot see, by architecture:**

- Your provider refresh or access tokens. They are in your Keychain, protected by
...
- The content of your mail, calendar, Notion pages, or Linear issues as it
  flows through a connector call. Connector requests go device -> provider,
  not through June API.

**Can see, and you should know it:**

- **Model inference is a separate path.** When a routine runs, its prompt (which
  can include mail, calendar, page, issue, or comment content the routine chose
  to read) goes to
...
1. **The connected provider.** You grant June's OAuth client the access shown
   by Google, Notion, or Linear. The provider sees the API calls June makes.
...
- The agent cannot read the token store while June's sandbox is engaged. The
  ...
  never a provider token. Signed rc builds verify this with both the `security`
  CLI and a direct `SecItemCopyMatching` probe before release.
- Connector tool descriptions mark mail, calendar, page, issue, and comment
  content as untrusted input because connected content can carry prompt
...
"Also revoke June's access" additionally calls the connected provider's revoke
